...
for port in `seq 6373 6378`; do
  ...
  sed -i s/\<K8S_PV_NFS_SERVER\>/${K8S_PV_NFS_SERVER}/g redis-cluster-node.pv-pvc.${REDIS_PORT}.yaml
  sed -i s/\<K8S_PV_NFS_PATH\>/${K8S_PV_NFS_PATH}/g redis-cluster-node.pv-pvc.${REDIS_PORT}.yaml
